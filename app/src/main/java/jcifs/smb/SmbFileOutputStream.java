...
public class SmbFileOutputStream extends OutputStream {
    ...
    SmbFileOutputStream(SmbFile file, boolean append, int openFlags) throws SmbException, MalformedURLException, UnknownHostException {
        ...
        file.open(openFlags, access | jcifs.smb.SmbConstants.FILE_WRITE_DATA, SmbFile.ATTR_NORMAL, 0);
        ...
        if ((file.tree.session.transport.server.capabilities & jcifs.smb.SmbConstants.CAP_LARGE_WRITEX) == jcifs.smb.SmbConstants.CAP_LARGE_WRITEX) {
            writeSizeFile = Math.min(jcifs.smb.SmbConstants.RCV_BUF_SIZE - 70, 0xFFFF - 70);
        } else {
