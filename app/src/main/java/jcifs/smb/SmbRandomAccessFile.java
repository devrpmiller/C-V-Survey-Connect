...
public class SmbRandomAccessFile implements DataOutput, DataInput {
    ...
    public SmbRandomAccessFile(SmbFile file, String mode)
        ...
        if (mode.equals("r")) {
            ...
            access = jcifs.smb.SmbConstants.FILE_READ_DATA | jcifs.smb.SmbConstants.FILE_WRITE_DATA;
        } else {
            throw new IllegalArgumentException("Invalid mode");
        }
        file.open(openFlags, access, SmbFile.ATTR_NORMAL, options);

        if (file.tree.session.transport.hasCapability(jcifs.smb.SmbConstants.CAP_LARGE_READX)) {
            readSize = Math.min(jcifs.smb.SmbConstants.RCV_BUF_SIZE - 70, 0xF000);
        } else {
            readSize = file.tree.session.transport.rcv_buf_size - 70;
        }

        if (file.tree.session.transport.hasCapability(jcifs.smb.SmbConstants.CAP_LARGE_WRITEX)) {
            writeSize = Math.min(jcifs.smb.SmbConstants.SND_BUF_SIZE - 70, 0xF000);
        } else {
